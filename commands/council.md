...
This command is a **themed layer** on the shared deliberation engine at `${CLAUDE_PLUGIN_ROOT}/commands/_council-engine.md`. All workflow logic, session management, and phase orchestration are defined in the engine. This file supplies the Council-specific configuration.
...
## Agent Roster (19 Perspectives + Maestro, v1.0.0)
...
| 17 | **Cipher** | Obsidian | Cryptographic engineering, protocol security, PQC | `council-cipher` | `Cipher` |
| 18 | **Warden** | Slate | OS kernel security, isolation, privilege boundaries | `council-warden` | `Warden` |
| 19 | **Prover** | Pearl | Formal methods, verification, security invariants | `council-prover` | `Prover` |
| — | **Steward** | Platinum | Orchestration, synthesis, facilitation (Maestro) | `council-steward` | *(not spawned)* |

The **Steward** (Maestro) is not a spawned agent — it is the facilitator persona the conductor adopts during sessions. See `${CLAUDE_PLUGIN_ROOT}/agents/council-steward.md` for the full persona.

Selection cap remains at **7 agents max** per session. The larger roster (19) gives more to choose from, not more in every session.

> **Held back for an `agentic-council-ee` spin-off:** Forge (microarchitecture/RTL security), Foundry (chip design/EDA flows), and Accountant (accounting/audit). Their personas and skills are not bundled in v1.0.0.

---

## Department Skills Tree

Skills ship flat under `${CLAUDE_PLUGIN_ROOT}/skills/<dept>-<skill>/SKILL.md` so Claude Code's plugin loader can discover them. The shared engine reads `${CLAUDE_PLUGIN_ROOT}/references/department-index.md` for routing.

```
${CLAUDE_PLUGIN_ROOT}/skills/
  architect-codebase-context/SKILL.md            # "Tech chief" — infra analysis + context briefing
  architect-schema-design/SKILL.md               # Database schema design
  architect-api-design/SKILL.md                  # REST/RPC endpoint contracts
  architect-distributed-patterns/SKILL.md        # Distributed system patterns
  skeptic-threat-model/SKILL.md                  # STRIDE-based threat analysis
  skeptic-failure-mode-analysis/SKILL.md         # Failure scenarios + mitigations
  skeptic-edge-case-enumeration/SKILL.md         # Systematic edge case discovery
  advocate-journey-mapping/SKILL.md              # User journey with states + emotions
  advocate-interaction-design/SKILL.md           # Component specs with all states
  advocate-i18n-review/SKILL.md                  # Internationalization readiness review
  advocate-a11y-audit/SKILL.md                   # WCAG 2.2 AA accessibility audit
  craftsman-testing-strategy/SKILL.md            # Test plan with coverage targets
  craftsman-pattern-analysis/SKILL.md            # Codebase pattern audit + conventions
  craftsman-e2e-testing/SKILL.md                 # E2E test design + visual regression
  scout-library-evaluation/SKILL.md              # Structured library scoring + comparison
  scout-competitive-analysis/SKILL.md            # Feature comparison matrix
  scout-technology-radar/SKILL.md                # Technology maturity assessment
  strategist-mvp-scoping/SKILL.md                # MoSCoW prioritization + value-effort matrix
  strategist-impact-estimation/SKILL.md          # RICE scoring for feature prioritization
  strategist-analytics-design/SKILL.md           # Telemetry events + A/B test instrumentation
  operator-deployment-plan/SKILL.md              # Deployment strategy + rollback procedures
  operator-observability-design/SKILL.md         # Monitoring, alerting, logging strategy
  operator-cost-analysis/SKILL.md                # Infrastructure cost modeling
  operator-finops-analysis/SKILL.md              # Cloud cost attribution + unit economics
  chronicler-documentation-plan/SKILL.md         # Documentation architecture + audiences
  chronicler-adr-template/SKILL.md               # Architecture Decision Record creation
  chronicler-changelog-design/SKILL.md           # Changelog + migration guide design
  guardian-compliance-review/SKILL.md            # GDPR/privacy compliance review
  guardian-data-classification/SKILL.md          # Data sensitivity classification
  guardian-audit-trail-design/SKILL.md           # Audit logging design
  tuner-performance-audit/SKILL.md               # Bottleneck identification + profiling
  tuner-caching-strategy/SKILL.md                # Cache hierarchy design
  tuner-load-modeling/SKILL.md                   # Capacity planning + benchmarks
  alchemist-schema-evaluation/SKILL.md           # Data warehouse schema design
  alchemist-pipeline-design/SKILL.md             # ETL/ELT pipeline architecture
  alchemist-ml-workflow/SKILL.md                 # ML workflow + experiment tracking
  pathfinder-platform-audit/SKILL.md             # Platform guideline compliance
  pathfinder-navigation-design/SKILL.md          # Mobile navigation + deep linking
  pathfinder-device-integration/SKILL.md         # Hardware APIs, sensors, biometrics
  artisan-visual-audit/SKILL.md                  # Structured visual design critique
  artisan-design-system-architecture/SKILL.md    # Token hierarchy + theming
  artisan-motion-design/SKILL.md                 # Animation principles + reduced-motion
  herald-growth-engineering/SKILL.md             # Onboarding funnels + referral systems
  herald-monetization-design/SKILL.md            # Pricing tiers + paywall architecture
  herald-messaging-strategy/SKILL.md             # Product copy + value propositions
  sentinel-embedded-architecture/SKILL.md        # Firmware design + RTOS patterns
  sentinel-protocol-design/SKILL.md              # BLE/MQTT/Matter protocol selection
  sentinel-fleet-management/SKILL.md             # Device provisioning + OTA updates
  oracle-prompt-engineering/SKILL.md             # System prompts + structured output
  oracle-rag-architecture/SKILL.md               # Chunking + embeddings + vector DB
  oracle-ai-evaluation/SKILL.md                  # Golden datasets + hallucination detection
  cipher-crypto-review/SKILL.md                  # Cryptographic implementation review
  cipher-protocol-analysis/SKILL.md              # Protocol state machine security analysis
  cipher-pqc-readiness/SKILL.md                  # Post-quantum cryptography readiness
  warden-isolation-review/SKILL.md               # Isolation boundary analysis + bypass testing
  warden-kernel-hardening/SKILL.md               # Kernel security configuration audit
  warden-hw-sw-boundary/SKILL.md                 # HW/SW security interface review
  prover-formal-spec/SKILL.md                    # TLA+ specification + model checking
  prover-invariant-analysis/SKILL.md             # Security invariant identification
```

**Total v1.0.0:** 19 departments × 2-4 skills each = **59 skills**.

Per-user skill-usage telemetry (recreated on first run): `~/.claude/council/registry.json`.
