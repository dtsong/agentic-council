...
### Preflight: Agent Teams Runtime Check

Before doing anything else, confirm the agent-teams primitive is enabled. The council relies on each member running as an isolated teammate; without this flag, deliberation cannot proceed.

1. Check whether `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` is set to `1` in the current shell environment (use `printenv CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` via Bash).
2. If unset or empty, print the following message verbatim and **EXIT** without proceeding:

```
⚠️  agentic-council requires the experimental agent-teams runtime.

Add this to your shell profile (~/.zshrc, ~/.bash_profile, etc.) and restart Claude Code:

    export CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1

Full setup: see INSTALL.md in the plugin repo.
```
...
Skills ship flat under `${CLAUDE_PLUGIN_ROOT}/skills/<dept>-<skill>/SKILL.md` (the plugin's read-only asset tree). Per-user telemetry lives at `~/.claude/council/registry.json` (created on first use).

```
${CLAUDE_PLUGIN_ROOT}/skills/<dept>-<skill>/SKILL.md   # Each skill, flat, kebab-case
${CLAUDE_PLUGIN_ROOT}/references/department-index.md   # Routing: dept → list of skills
~/.claude/council/registry.json                        # Per-user skill usage tracking (writable)
```

The engine reads `${CLAUDE_PLUGIN_ROOT}/references/department-index.md` once per session to learn which skills belong to which department. See the themed command file for the full skill tree listing.
...
4. Read `~/.claude/council/registry.json` for previously-used skills (create empty `{}` file if missing)
...
1. For each selected agent, look up their department in `${CLAUDE_PLUGIN_ROOT}/references/department-index.md` to get the list of skills available (each maps to `${CLAUDE_PLUGIN_ROOT}/skills/<base-agent>-<skill>/SKILL.md`)
...
7. Update `~/.claude/council/registry.json` (create with `mkdir -p` and `{}` if missing):
